...
import stargateBack.dao.arduino.listener.RxtxInputListener;
import stargateBack.dao.bean.LedTransfert;
...
public class RxtxConnection {
	...
	private Queue<LedTransfert> ledTx;

	private static final String TX_VIDE = "TXVIDE";
	private Gson gson;

	/**
	 * The port we're normally going to use.
	 */
	private static String PORT_NAMES[] = { "/dev/tty.usbmodem1421", // Mac OS X 10.8
			"/dev/tty.usbmodem1411", // Mac OS X 10.9
			"/dev/ttyUSB0", // Linux
			"/dev/ttyACM0", // debian/raspbian
			"/dev/ttyAMA0", // debian/raspbian
			"/dev/ttyS1", "COM3", // Windows
	};

	/**
	 * A BufferedReader which will be fed by a InputStreamReader converting the
	 * bytes into characters making the displayed results codepage independent from
	 * arduino website
	 */
	...
	private void initialize()
		...
		if (portEnum.hasMoreElements()) {
			...
			logger.error(
					"No port found. Linux user: don't forget that only ports like this: /dev/ttyS* will be scanned");
		}
	...
	public void AddToTransfert(ArrayList<LedTransfert> ledTransfert) {

		if (ledTx == null) {
			ledTx = new LinkedList<LedTransfert>();
		}
		ledTx.addAll(ledTransfert);
	}
	...
	protected void fireLineChanged(String oldLine, String newLine) {
		...
		// Cas 1 réponse configuration

		// Cas 2 reception top attente commande
		// => regarder dans ledTx

		// si il y a des donnés les envoyer sur le serial
		try {
			while (!ledTx.isEmpty()) {
				LedTransfert transfert = ledTx.remove();
				// TODO jsonifier ?

				String o = gson.toJson(transfert);

				getOutput().write(o.getBytes());

			}

			// => envoyer un OK vide
			String o = gson.toJson(TX_VIDE);
			getOutput().write(o.getBytes());
